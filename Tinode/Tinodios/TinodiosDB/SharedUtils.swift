...
public class SharedUtils {
    ...
    #if DEBUG
        public static let kHostName = "stg-tinode.bluebird.id:443" // localhost
    ...
    public static func saveUserId(userUd: String) {
        SharedUtils.kAppDefaults.set(userUd, forKey: "userId")
    }
    
    public static func getUserId() -> String {
        return SharedUtils.kAppDefaults.string(forKey: "userId") ?? ""
    }
    
    public static func removeUserId() {
        SharedUtils.kAppDefaults.removeObject(forKey: "userId")
    }
    
    public static func saveToken(token: Token) {
        if let encodedToken = try? JSONEncoder().encode(token) {
            SharedUtils.kAppDefaults.set(encodedToken, forKey: "bbToken")
        }
    }

    public static func getToken() -> Token? {
        if let savedTokenData = SharedUtils.kAppDefaults.data(forKey: "bbToken"),
           let decodedToken = try? JSONDecoder().decode(Token.self, from: savedTokenData) {
            return decodedToken
        }
        return nil
    }
    ...
    public static func removeToken() {
        SharedUtils.kAppDefaults.removeObject(forKey: "bbToken")
    }
...
extension Tinode {
    public static func getConnectionParams() -> (String, Bool) {
        ...
        case .regress, .dev, .staging:
            return ("stg-tinode.bluebird.id:443", true)
        default:
            return ("stg-tinode.bluebird.id:443", true)
        }
