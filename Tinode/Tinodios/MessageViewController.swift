...
public class MessageViewController: UIViewController {
    ...
    private var navBarBackBtn: UIBarButtonItem = {
        guard let closeIcon = UIImage(named: "ic-arrow-left-black", in: .main, compatibleWith: nil)?.withRenderingMode(.alwaysOriginal) else {
            fatalError("closeIcon not found")
        }
        return UIBarButtonItem(image: closeIcon, style: .plain, target: self, action: #selector(backButton(sender:)))
    }()
    ...
    let backgroundImageView: UIImageView = {
       let theImageView = UIImageView()
        theImageView.image = UIImage(named: "chat_background", in: .main, with: nil)
    ...
    @objc func backButton(sender: UIMenuController) {
        ...
        //BIRDTALKSDKTrackerModel.sharedInstance.logEvent(.backFromChatScreen(userId: userId, orderId: callRoomId ?? "", roomId: topicName ?? ""))
...
//            heightPredefinedConstraint?.isActive = true
    ...
    public override func viewDidLoad() {
        ...
        //BIRDTALKSDKTrackerModel.sharedInstance.logEvent(.openChatSuccess(userId: Cache.getUserId(), roomId: topicName ?? "", orderId: callRoomId ?? ""))
...
//        self.navigationItem.setRightBarButtonItems(items, animated: false)
    ...
    func displayChatMessages(messages: [StoredMessage], _ scrollToMostRecentMessage: Bool) {
        ...
        //BIRDTALKSDKTrackerModel.sharedInstance.logEvent(.receiveMessage(senderName: "", orderId: callRoomId ?? "", roomId: topicName ?? "", messageId: ""))
